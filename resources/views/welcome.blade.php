    ...
    <div class="services-wrapper bg-white py-5">
        <div class="container">
            <div class="row main-slider">
                <div class="col-md-12">
                    <div class="service">
                        <img class="d-block d-sm-block d-xs-block d-lg-none "
                             src="{{asset(App\HomePageBanner::where('locale',App::getLocale())->first()->small_image)}}">
                        <img class="d-none d-sm-block "
                             src="{{asset(App\HomePageBanner::where('locale',App::getLocale())->first()->image)}}">
                        <h3 class="text-center d-block d-sm-block d-xs-block d-lg-none banner-text"
                            ...
                            @guest
                                <a href="{{route('register')}}"
                                   class="btn btn-lg btn-light rd-in mt-3 primary-color-rev"> {{App\HomePageBanner::where('locale',App::getLocale())->first()->button_text}} </a>
                            @endguest
                        </h3>
                        <h3 class="text-center d-none d-sm-block banner-text">{{App\HomePageBanner::where('locale',App::getLocale())->first()->text_1}}
                            <br>{{App\HomePageBanner::where('locale',App::getLocale())->first()->text_2}}<br>
                            @guest
                                <a href="{{route('register')}}"
                                   class="btn btn-lg btn-light rd-in mt-3 primary-color-rev"> {{App\HomePageBanner::where('locale',App::getLocale())->first()->button_text}} </a>
        ...
        <div class="container">
            <div class="row">
                @foreach (App\HomePageProviderType::all() as $category)
                    <div class="col-md-12">
                        <p style="font-weight: 800;font-size:1.3rem; color:#241332"
                           class="pb-0 mb-1">{{_ti($category->providerType->name)}}
                            <a href="{{route('FilterByType',$category->provider_type_id)}}"
                               style="color:#04808B; font-weight:800" class="float-right">
                                    ...
                                    <div class="freelancer">
                                        <div>
                                            ...
                                            @if(auth()->user()->favorits->where('provider_id',$provider->id)->count() != 0) favorit @endif
                                                        " @if(auth()->user()->favorits->where('provider_id',$provider->id)->count()
                                        != 0)
                                                          style="color: rgb(212, 127, 166);"
                                                          @endif id="heart_{{$provider->id}}"
...
@section('script')
    <script>
        $(document).ready(function () {
            ...
            fetch("https://cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js").then(function (response) {
